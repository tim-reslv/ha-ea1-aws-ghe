resource "aws_ec2_transit_gateway_route_table_propagation" "transit_gateway_route_table_propagations" {
  for_each = {
    for key, value in var.transit_gateway_route_table_propagations : key => value
    if value.type == "vpc"
  }
...
resource "aws_ec2_transit_gateway_route_table_propagation" "vpn_transit_gateway_route_table_propagations" {
  for_each = {
    for key, value in var.transit_gateway_route_table_propagations : key => value
    if value.type == "vpn"
  }
